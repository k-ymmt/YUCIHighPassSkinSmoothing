...
+ (CIColorKernel *)filterKernel {
    ...
    dispatch_once(&onceToken, ^{
        NSString *kernelString = [[NSString alloc] initWithContentsOfURL:[SWIFTPM_MODULE_BUNDLE URLForResource:NSStringFromClass([YUCIGreenBlueChannelOverlayBlend class]) withExtension:@"cikernel"] encoding:NSUTF8StringEncoding error:nil];
...
+ (CIColorKernel *)filterKernel {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *kernelString = [[NSString alloc] initWithContentsOfURL:[SWIFTPM_MODULE_BUNDLE URLForResource:NSStringFromClass([YUCIHighPassSkinSmoothingMaskBoost class]) withExtension:@"cikernel"] encoding:NSUTF8StringEncoding error:nil];
        kernel = [CIColorKernel kernelWithString:kernelString];
    });
    return kernel;
}
